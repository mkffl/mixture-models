...
def test_mixture_m_step_poisson(global_vars):
    ...
    result = mixture_m_step_poisson(X=X, 
                                    q=q, 
                                    C=q.shape[1], 
                                    D=X.shape[1])
    ...
    lambda_expected = np.array([X_mean, 
                                X_mean])
...
def test_m_step_poisson(global_vars):
    ...
    result = m_step_poisson(X=global_vars["X"], 
                            q=q)
...
def test_poisson_likelihood(global_vars):
    ...
    # Seek the probability of no event occuring
    ...
    pmf_expected = np.ones(shape=(global_vars["num_rows"], 2)) * proba_special_case
